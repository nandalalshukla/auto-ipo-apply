describe("Meroshare Automation", () => {
  ...
  before(() => {
    ...
    cy.wait("@applicableshare").then((resp) => {
      ...
      applicableIpos = response.filter((data) => {
        if (
          !data.action &&
      ...
      if (applicableIpos.length === 0) {
        cy.log("No IPO to apply");
        cy.screenshot("no-ipo-to-apply", { capture: "fullPage" });
        cy.writeFile("cypress/results/run-status.json", {
          status: "NO_IPO",
          message: "No IPO to apply",
          eligibleCount: 0,
        ...
        cy.writeFile("cypress/results/run-status.json", {
          status: "IPO_FOUND",
          message: `Found ${applicableIpos.length} IPO(s) to apply`,
          eligibleCount: applicableIpos.length,
          timestamp: new Date().toISOString(),
        });
      }
    });
  ...
  it("Apply for Share", () => {
    if (applicableIpos.length === 0) {
      cy.log("No IPO to apply. Screenshot proof captured on My ASBA page.");
      return;
    }

    applicableIpos.forEach((data) => {
      cy.visit("/");
      cy.get("li.nav-item")
        .contains("My ASBA", {
          matchCase: false,
